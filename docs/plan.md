...
### mise.toml とプロジェクトツール検出
- **役割**: `mise.toml` は ecro 自体のエディタ設定ではなく、プロジェクトの開発ツール環境として扱う
- **検出対象**: `[tools]` から `clojure`、`java`、`graalvm`、`rust`、`rust-analyzer` などを検出する
- **用途**: LSP、tree-sitter、analyzer の候補選択に使う。ただし実行前に実行ファイルの有無は必ず確認する
- **優先順位**: buffer の構文モードはファイル拡張子を第一候補にし、`mise.toml` はプロジェクト全体の補助シグナルとして使う
- **非対象**: keybinding、theme、editor behavior、mode設定は `mise.toml` ではなく `~/.ecro/init.edn` / `~/.ecro/init.clj` に置く
- **将来拡張**: project-local editor config が必要になった場合は `.ecro/config.edn` または `.ecro/init.clj` を検討する
...
### Phase 12: mise project tooling integration
**目標**: `mise.toml` からプロジェクトのツール情報を検出し、LSP・tree-sitter・analyzer候補選択の土台を作る

| テスト | コミット |
|-------|---------|
| `mise.toml` の `[tools]` を読み込める | feat: parse mise tool config |
| tool名を keyword set として正規化できる | feat: normalize mise tools |
| tool情報から analyzer / LSP 候補を導出できる | feat: infer analyzer candidates from mise tools |
| `mise.toml` が無い場合は空の検出結果を返す | feat: handle missing mise config |
| `mise.toml` を editor config として扱わないことをテストする | feat: separate mise tooling from editor config |

### Phase 13: mode system
...
### Phase 14: tree-sitter syntax adapter
...
### Phase 16: indentation / structural editing
**目標**: インデントと構造的編集
